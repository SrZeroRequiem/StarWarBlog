...
export const StarshipProfile = () => {
    ...
    function getPilots(pilots) {
        if (pilots[0] !== undefined) {
            return pilots.map((pilotId) => {
                const pilot = store.characters.find((char) => char.pk === pilotId);
                return pilot ? (
                    <span key={pilot.pk} className="ms-1 mb-1">
                        <Link to={"/character/" + pilot.pk}>{pilot.name}</Link>,
                    </span>
                ) : null;
            });
        } else {
            return <span className="ms-1 mb-1">Unknown</span>;
        }
    }
    ...
    useEffect(() => {
        const fetchStarshipDetails = async () => {
            try {
                const starshipDetails = await actions.getEntityById(params.theid, "starships");
            ...
            <section className="d-flex justify-content-center mt-5 p-0">
                <main className="single-card col-12 row px-0">
                    <figure className="col-lg-8 col-12 p-0 m-0 d-flex" style={{ height: "-webkit-fill-available" }}>
                        <img
                            src={`https://raw.githubusercontent.com/tbone849/star-wars-guide/master/build/assets/img/starships/${params.theid}.jpg`}
                            className="img-card"
                            onError={(e) => {
                ...
                {details !== undefined ? (
                    <article className="row w-100 stats-tab">
                                    ...
                                    if (starshipFilms.length === 1) {
                                        return (
                                            <span key={film.pk} className="ms-1 mb-1">
                                                <Link to={"/film/" + film.pk}>{"Star Wars: " + film.title + ` (Episode ${actions.romanize(film.episode_id)})`}</Link>,
                                            </span>
                                        );
                                    } else return null;
                                })}
                        ...
                        <div className="detail-tab-item col-6 d-flex flex-column col-lg">
                            ...
                            <span className="ms-1 mb-1">{`Cost: ${actions.stringFormat(details.cost_in_credits)} C`}</span>
                            <span className="ms-1 mb-1">{`Model: ${details.model}`}</span>
                            <span className="ms-1 mb-1">{`Class: ${actions.stringFormat(details.starship_class)}`}</span>
